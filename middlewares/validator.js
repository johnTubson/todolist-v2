...
function listId_validator(medium) {
	if (!medium) throw new ReferenceError("Validator medium not specified, medium housing value to be validated required!");
	let spec;
	if (medium === "body") {
		spec = "body";
	...
	return async function (req, res, next) {
		...
		if (!result) {
			return next(new ParamError("Resource not found", 400));
		}
		return next();
	};
}
