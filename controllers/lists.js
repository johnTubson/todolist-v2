...
async function getLists(req, res) {
  ...
  let selected_list = lists.find((list) => {
    return (
      list._id.toString() ===
      (req.params.list_id ? req.params.list_id : req.user.default_list)
    );
  });
...
// async function getLists(req, res) {
//   const lists = await List.find({ user_id:"6488604f42390d2d103a8d1b" });
//   res.locals.lists = lists;
//   let selected_list = lists.find((list) => {
//     return (
//       list._id.toString() ===
//       (req.params.list_id ? req.params.list_id : "64940e2be427b4373a17007f")
//     );
//   });
//   res.locals.selected_list = selected_list;
//   res.locals.default_list = (selected_list._id.toString() === "64940e2be427b4373a17007f") ? true : false;
//   res.render("lists");
// }
...
//   const list = req.body.list_id;
//   if(!list) return new AppError("List not found", 400);
//   const user = await User.findById("6488604f42390d2d103a8d1b");
//   if(!user) return new AppError("Unauthorised operation", 401)
//   user && (user.default_list = list);
//   await user.save();
//   // req.session.passport.user.default_list = list;
